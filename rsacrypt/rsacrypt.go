...
func Encrypt(keybytes []byte, infile string, outfile string, outkeyfile string) error {
	...
	_, err = rand.Read(key)
	if err != nil {
		return fmt.Errorf("Unable to generate randon key. %s", err)
	}

	// encrypt input file using AES key
	cipherfile, err := encrypt(inbytes, key)
	if err != nil {
		return fmt.Errorf("Unable to encrypt. %s", err)
	}

	// write encrypted file to output file
	err = ioutil.WriteFile(outfile, cipherfile, 0600)
	if err != nil {
		return fmt.Errorf("Unable to write to output file. %s", err)
	}

	// encrypt key with rsa publick key
	cipherkey, err := rsa.EncryptOAEP(sha1.New(), rand.Reader, pubkey.(*rsa.PublicKey), key, []byte(""))
	if err != nil {
